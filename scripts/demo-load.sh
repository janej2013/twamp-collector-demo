...
# DURATION (default 60s), then clean up. Watch it at localhost:3000.
set -euo pipefail
cd "$(dirname "$0")/.."

DURATION=${DURATION:-60s}

./bin/collector -listen 127.0.0.1:8620 -metrics-addr 127.0.0.1:9090 \
  -normal-cap 64 -high-cap 16 -workers 1 >/dev/null 2>&1 &
COL=$!
trap 'kill -TERM $COL 2>/dev/null || true; wait $COL 2>/dev/null || true' EXIT
sleep 0.5

echo "sending bursty traffic for $DURATION — watch http://localhost:3000"
